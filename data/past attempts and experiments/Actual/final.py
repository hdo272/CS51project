...
# def filterSHARP(im):
    ...
    # im1.save("sharpened" + ext)

# applying the filter and saving it in a variable
# filterSHARP(im1)


# LOCALIZE

# making arrays from sharpened image 
img_rgb = cv2.imread(sys.argv[1])
img = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
w, h = img.shape[0], img.shape[1]


# defining the variables for localization
...
if num == 0:
    ...
    for loc in results:
        ...
        imagename = 'output' + str(i) + '.png'

        # CUT
        imge = cv2.imread(imagename)
        imgc = cv2.cvtColor(imge, cv2.COLOR_BGR2GRAY)
        hc , wc = imgc.shape[0], imgc.shape[1]
        ...
        for x in range(0, wc):
            for y in range(0, hc):
                if imgc.item((y,x)) < avg:
                    imgc.itemset((y,x), MAX_PIXEL)
                    if y == middle_h:
                        total += MAX_PIXEL
                else:
                    imgc.itemset((y,x), 0)
        ...
        cut_image = imgc[down_bound:up_bound]
        ...
        for label in range(1, num + 1):
            ...
            if (array.size < min_size
                    ...
                    or array.shape[1] < min_width):

                # or array.shape[0] == new_h):
              print array.shape[0]
              cut_image[loc] = 0
        ...
        # import image (REMEMBER TO REPLACE cut in pic)
        pic = cv2.imread('threshed_cut.png')
        ...
        for i in range(0, wnum):
            ...
            seg = img[:,b:e]
            cv2.imwrite(str(num) + str(i) + '.png', seg)
            segname = str(num) + str(i) + '.png'
            segim = Image.open(segname)  # Open image object using PIL
            print pytesser.image_to_string(segim)     # Run tesseract.exe on image
            print pytesser.image_file_to_string(segname)
