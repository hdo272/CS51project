...
if num == 0:
    ...
    for loc in results:
       ...
       #SHARPEN
       imgname = 'output' + str(i) + '.png'
       im1 = Image.open(imgname)

       # define the sharpen filter for the image
       def filterSHARP(im):
          im1 = im.filter(ImageFilter.SHARPEN)
          im1.save('sharp'+ str(i) + '.png')

       # use the filter
       filterSHARP(im1)
       
       sharp = 'sharp'+ str(i) + '.png'
       
       #THRESHOLD
       sharpimg = cv2.imread(sharp)
       img = cv2.cvtColor(sharpimg, cv2.COLOR_BGR2GRAY)
